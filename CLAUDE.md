...
- **M9:** Pre-submission adversarial review fixes — essentially complete. The 2026-07-28 review graded the manuscript 65/100 and found two blocking defects, both now fixed: (1) "linear interpolation" actually called RiverLoad `method1`, a mean-C × mean-Q averaging estimator — switched to `method6` (true interpolation) and `calculate_pw` renamed **`calculate_li`**, method key `'pw'` → `'li'` across 9 files; (2) the x-axis tick labels on Figs 7–10 were wrong (the "Weekly" tick was 4-day sampling) — breaks corrected to true intervals. Full rerun completed clean (80.8 min, 0 failures). ~45 text corrections applied to the .docx, Results/Discussion reorganized, MacroSheds methods split with a new **Table 2** for the decision rules, 19 of 23 figures refreshed in-place. **M9a–M9d are complete; 10 smaller items remain open in M9b/M9c/M9e** — see `plans/m1_planning.md`. The notable ones: three figure assets (`figa9_macrosheds_density`, `figa9_macrosheds_method_comp`, `fig11_hbef_method_ts`) exist but are still never referenced in the text; WRTDS is computed into `load_annual.csv` but undocumented in Methods; and the Data Availability section still sits before the Introduction. Nic-owned: figshare re-upload + DOI bump, and manual replacement of Figures 2 and 4 in Word (M10d).
