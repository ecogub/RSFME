...
for(solute_var in c('IS_NO3', 'IS_spCond')){
    ...
    if(target_solute == 'IS_spCond'){save(out_tbl, file = here('data','coarsen_hbef', '100reps_annual_Ca.RData'))}
    if(target_solute == 'IS_NO3'){save(out_tbl, file = here('data','coarsen_hbef', '100reps_annual_NO3.RData'))}
}
